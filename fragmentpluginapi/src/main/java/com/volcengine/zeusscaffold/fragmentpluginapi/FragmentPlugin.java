...
import com.volcengine.zeus.fragmentplugin_api.BuildConfig;
 ...
 * @date 8/31/21
 */
public class FragmentPlugin {
    public static IFragmentPluginApi api = null;
    ...
    public static boolean preparePlugin(Context context, OnPluginInstallListener listener) {
        if (!Zeus.isPluginInstalled(pluginPkgName)) {
            Toast.makeText(context, "PluginFragment未安装，不可用，注册安装的回调", Toast.LENGTH_SHORT).show();
            ...
            if (Zeus.isPluginLoaded(pluginPkgName)) {
                ...
                if (loadPlugin) {
                    Toast.makeText(context, "PluginFragment加载成功，可用", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(context, "PluginFragment加载失败，不可用", Toast.LENGTH_SHORT).show();
                }
                return loadPlugin;
            }
        }
     ...
     * 通过Spi的形式访问插件
     */
    public static IFragmentPluginApi getApi() {
        if (api == null) {
            return emptyImpl;
        }
        return api;
    ...
    private static final IFragmentPluginApi emptyImpl
            = fragment -> Toast.makeText(fragment.requireContext(), "PluginFragment未加载", Toast.LENGTH_SHORT).show();
